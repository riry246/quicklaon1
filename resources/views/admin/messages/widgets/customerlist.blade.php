<div class="chat-info border">
    ...
    <div class="d-grid align-items-top p-3 border-bottom">
        <button class="btn btn-success d-flex align-items-center justify-content-center" data-bs-toggle="modal"
            ...
            <i class="ri-add-circle-line fs-16 align-middle me-1"></i>Send to All
        </button>
    </div>
        ...
        <div class="tab-pane fade show active border-0 chat-users-tab" id="users-tab-pane" role="tabpanel"
            ...
            <ul class="list-unstyled mb-0 mt-2 chat-users-tab" id="chat-msg-scroll">
                ...
                @foreach ($groupedMessages as $k => $m)
                    ...
                    @if($k)
                        <a href="javascript:void(0);" onclick="message({{ $k }})">
                    @else
                    <a href="javascript:void(0);" onclick="message(0)">
                    @endif
                            <div class="d-flex align-items-top">
                                <div class="me-1 lh-1">
                                    <span
                                        class="avatar avatar-sm avatar-rounded bg-success-transparent fw-semibold me-2">
                                        {{ substr($loan_helper->getUserName($k) ?? 'Unknown', 0, 1) }}
                                    </span>

                                </div>
                                <div class="flex-fill">
                                    <p class="mb-0 fw-semibold">
                                        {{ $loan_helper->getUserName($k) ?? 'Unknown' }}

                                    </p>

                                    <p class="fs-12 mb-0">
                                        <span class="chat-msg text-truncate">{{ $m->subject }}</span>
                                        <span
